...
scaler = RobustScaler()
x_train_encoded = pd.get_dummies(X_train, columns=['smoker_yes'], drop_first=True, dtype=int)
X_train_scaled = scaler.fit_transform(x_train_encoded)
KNN_trained = knn_model.fit(X_train_scaled, y_train)
knn_cv_scores = cross_val_score(KNN_trained, X_train_scaled, y_train, cv=5, scoring='r2')
x_test_encoded = pd.get_dummies(X_test, columns=['smoker_yes'], drop_first=True, dtype=int)
...
knn_test_score = KNN_trained.score(x_test_scaled, y_test)
